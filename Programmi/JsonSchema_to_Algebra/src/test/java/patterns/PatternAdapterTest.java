...
public class PatternAdapterTest {
	 ...
	 * @Test
	 * 
	 * @Ignore // This is not implemented yet. public void testMultiBound() throws
	 * REException { assertEquals("foo|bar", PatternAdapter.rewrite("^foo$|^bar$"));
	 * assertEquals("foo@|bar@", PatternAdapter.rewrite("^foo|^bar"));
	 * assertEquals("@foo|bar@", PatternAdapter.rewrite("foo$|^bar"));
	 * assertEquals("foo|@bar", PatternAdapter.rewrite("^foo$|bar")); }
	 * 
	 * 
	 * 
	 * @Test public void testPreserveEscapings() throws REException { // Must
	 * preserve "\.". assertEquals("\\.", PatternAdapter.rewrite("^\\.$"));
	 * 
	 * // Must preserve "\+". assertEquals("\\+", PatternAdapter.rewrite("^\\+$"));
	 * 
	 * // Must preserve "\*". assertEquals("\\*", PatternAdapter.rewrite("^\\*$"));
	 * 
	 * // Must preserve "\\".
	 ...
	 * 
	 * 
	 * @Test public void testEscapeTabs() throws REException { assertEquals("\t",
	 * PatternAdapter.rewrite("^\\t$")); assertEquals("\n",
	 * PatternAdapter.rewrite("^\\n$")); }
	 * 
	 * 
	 * @Test public void testEscapeAtSymbol() throws REException {
	 * assertEquals("foo\\@bar", PatternAdapter.rewrite("^foo@bar$"));
	 * //assertEquals("@foo\\@bar@", PatternAdapter.rewrite("^foo@bar$")); // TODO
	 * assertEquals("([a-z])\\@([0-9])", PatternAdapter.rewrite("^[a-z]@[0-9]$")); }
	 * 
	 * 
	 * @Test public void testDigits() throws REException { assertEquals("[0-9]",
	 * PatternAdapter.rewrite("^\\d$")); assertEquals("\\\\d",
	 * PatternAdapter.rewrite("^\\\\d$")); assertEquals("(a|[0-9]|a)",
	 * PatternAdapter.rewrite("^[a\\d]$")); // TODO - why 'a' twice?
	 * assertEquals("foo([0-9])bar", PatternAdapter.rewrite("^foo[\\d]bar$"));
	 * assertEquals("foo(a|b|c|]|d|e|f|[0-9]|f)bar",
	 * PatternAdapter.rewrite("^foo[abc\\]def\\d]bar$")); }
	 * 
	 * 
	 * @Test public void testCharClasses() throws REException {
	 * assertEquals("([0-9])", PatternAdapter.rewrite("^[0-9]$"));
	 * assertEquals("(([0-9]))+", PatternAdapter.rewrite("^[0-9]+$"));
	 * assertEquals("(([0-9]))+\\+([0-9])",
	 * PatternAdapter.rewrite("^[0-9]+\\+[0-9]$")); assertEquals("(\\.|\\+|-)",
	 * PatternAdapter.rewrite("^[.+-]$"));
	 * 
	 * assertEquals("([\\.]|\\+|-)", PatternAdapter.rewrite("^[\\.+-]$"));
	 * assertEquals("(\\\\|\\.)", PatternAdapter.rewrite("^[\\\\.]$")); }
	 * 
	 * 
	 * @Test public void testGroupNoncapturing() throws REException {
	 * assertEquals("foo(bar)?", PatternAdapter.rewrite("^foo(?:bar)?$")); }
	 * 
	 * 
	 * 
	 * @Test public void testParentheses() throws REException {
	 ...
	 * 
	 * 
	 * @Test public void testMultiplicity() throws REException {
	 * assertEquals("(a){3}", PatternAdapter.rewrite("^a{3}$"));
	 * assertEquals("(a){2,5}", PatternAdapter.rewrite("^a{2,5}$")); }
	 * 
	 * 
	 * @Test public void testAnyWordCharacter() throws REException {
	 * assertEquals("[a-zA-Z0-9_]", PatternAdapter.rewrite("^\\w$"));
	 * assertEquals("([a-zA-Z0-9_])", PatternAdapter.rewrite("^[\\w]$")); }
	 * 
	 * 
	 * @Test public void testBackslashInRange() throws REException {
	 * assertEquals("(a|\\\\)", PatternAdapter.rewrite("^[a\\\\]$"));
	 * assertEquals("(\\\\|a)", PatternAdapter.rewrite("^[\\\\a]$")); }
	 */
